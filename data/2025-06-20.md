...
<details>
  <summary>Details</summary>
Motivation: Addressing the underexplored intersection of continual learning (CL) and cooperative multi-agent settings by providing a scalable benchmark.

Method: Introduces MEAL, leveraging JAX for GPU acceleration to overcome computational bottlenecks in existing CL benchmarks.

Result: Naive CL and MARL methods perform well on simple environments but fail in complex settings requiring coordination and adaptation.

Conclusion: MEAL identifies critical architectural and algorithmic features for effective CMARL, advancing research in this niche.
...
<details>
  <summary>Details</summary>
Motivation: PPO is slow for long-response LLMs due to on-policy nature and idle hardware during rollouts.

Method: Introduces T-PPO with Extended GAE for advantage estimation and a mechanism for independent policy/value optimization.

Result: T-PPO achieves up to 2.5x training efficiency gain and outperforms competitors on AIME 2024.

Conclusion: T-PPO is a scalable and efficient alternative to PPO for training reasoning LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Traditional heuristic designs for CO problems rely on manual expertise and lack generalization.
...
Result: Outperforms LLM-based hyper-heuristics and matches/exceeds specialized solvers in benchmarks.

Conclusion: HeurAgenix demonstrates robust performance and flexibility in solving CO problems.
...
<details>
  <summary>Details</summary>
Motivation: Autonomous coordination in multi-satellite EO missions is challenging due to real-time demands and resource constraints, requiring advanced RL/MARL solutions.

Method: The study models single-satellite operations and extends to multi-satellite constellations using MARL (PPO, IPPO, MAPPO, HAPPO) in a near-realistic simulation.

Result: MARL effectively balances imaging and resource management, handling non-stationarity and reward interdependency in decentralised coordination.

Conclusion: The study provides a foundation for autonomous satellite operations, offering practical guidelines for policy learning in EO missions.
...
<details>
  <summary>Details</summary>
Motivation: The increasing demands of maritime IoT tasks and challenges in computation offloading and resource allocation due to uncertainty motivate the need for an efficient framework.

Method: A cooperative MEC framework is proposed, leveraging Lyapunov optimization for uncertain tasks and reformulating the problem into a Markov game solved by a heterogeneous-agent soft actor-critic.

Result: Simulations confirm the framework's effectiveness in optimizing computation offloading and resource allocation.

Conclusion: The proposed approach successfully addresses maritime IoT challenges by combining UAV-vessel cooperation, Lyapunov optimization, and advanced reinforcement learning.
...
<details>
  <summary>Details</summary>
Motivation: Existing navigation methods process historical observations simultaneously, ignoring internal data associations, which limits performance. The paper addresses this by analyzing navigation tasks causally.
...
Result: Empirical evaluations show CAN outperforms baselines across tasks and environments, with gains attributed to the Causal Understanding Module.

Conclusion: CAN generalizes effectively in Reinforcement and Supervised Learning settings without added computational cost, demonstrating the value of causal insights in navigation.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of orchestrating multiple AI models in FA labs into cohesive workflows, enhancing efficiency and integration.
...
Conclusion: The LPA successfully integrates AI components into FA workflows, improving automation and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To systematically improve LLM agents' decision-making in dynamic settings by clarifying the impact of state representation on behavior and enhancing study comparability.

Method: A framework categorizing state representation methods along three axes: action informativeness, reward informativeness, and prompting style, applied to a dynamic selfish routing game.

Result: Summarized history, regret-based rewards, and limited action information lead to behavior closer to game-theoretic equilibrium, while other representations cause deviations or instability.

Conclusion: State representation significantly influences LLM agent behavior, with specific methods yielding more stable and equilibrium-aligned outcomes.
...
Conclusion: The AI Policy Module effectively prepares students for AI policy challenges, with the AI Regulation Assignment being a key tool.
...
<details>
  <summary>Details</summary>
Motivation: LRMs often generate unnecessarily verbose content, increasing inference costs and reducing efficiency. The study aims to uncover and exploit the models' inherent potential for concise reasoning.

Method: Two methods are introduced: Efficiency Steering (a training-free activation technique) and Self-Rewarded Efficiency RL (a reinforcement learning framework). Both aim to reduce reasoning length while maintaining accuracy.

Result: Experiments on seven LRM backbones show significant reduction in reasoning length without losing performance, demonstrating the effectiveness of the proposed methods.

Conclusion: The study highlights that LRMs can achieve better efficiency by leveraging their intrinsic capabilities through self-guided techniques, offering practical improvements for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing agentic systems lack full autonomy, limiting adaptability and scalability. SwarmAgentic aims to address these gaps by enabling from-scratch agent generation and self-optimization.

Method: SwarmAgentic constructs agentic systems from scratch, optimizes functionality and collaboration via language-driven exploration, and evolves candidate systems using feedback-guided updates inspired by PSO.
...
Result: Revealed performance gaps between AI systems and humans in tasks requiring cross-domain reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Practical implementations of the Ising model are hindered by soft constraints that either slow down mixing or fail to enforce feasibility, motivating the need for a better algorithm.

Method: The authors extend parallel tempering (PT) by adding a second dimension of replicas that interpolate penalty strengths, ensuring constraint satisfaction in low-energy states.

Result: 2D-PT improves mixing in constrained replicas, achieves near-ideal mixing in graph sparsification, and provides significant speedup over conventional PT.

Conclusion: 2D-PT is broadly applicable to constrained Ising problems and can be deployed on existing Ising machines, offering a practical solution to constraint handling.
...
Conclusion: NetraAI represents a new generation of adaptive, self-reflective AI for clinical discovery, aligning with concept-level reasoning paradigms like JEPA.
...
Method: ETS integrates EEG with synchronized eye-tracking data for tasks like text generation and sentiment classification.

Result: Superior performance in BLEU/Rouge scores for EEG-to-text and 10% F1 score improvement in sentiment classification, with cross-subject robustness.

Conclusion: ETS shows promise for high-performance open-vocabulary EEG-to-text systems, handling diverse data sources effectively.
...
<details>
  <summary>Details</summary>
Motivation: Traditional direct measurements of onflow parameters face challenges like sensor faults. A data-driven, real-time learning approach is needed for applications like wind tunnel monitoring and aerodynamic performance tracking.

Method: A ConvNet is trained offline for core prediction, then transfer learning is applied by retraining selected layers. The method is tested for domain and task adaptation using steady CFD data.
...
Conclusion: Transfer learning with ConvNets is a viable solution for predicting onflow parameters, though noise robustness remains a limitation.
...
Conclusion: PIPE effectively bridges the gap in utilizing visual data for forecasting, demonstrating significant advancements in accuracy and alignment.
...
<details>
  <summary>Details</summary>
Motivation: The demand for efficient, high-density storage systems is growing, but multi-deep AVS/RS face retrieval challenges due to lane blockages. Current homogeneous storage strategies limit flexibility.

Method: A graph-based state representation integrates item attributes and warehouse topology. A neural network combines GNN for local embeddings and Transformer for global priority assignments.

Result: The proposed architecture outperforms heuristic methods, effectively minimizing retrieval tardiness and generalizing to diverse layouts.

Conclusion: The framework successfully addresses retrieval challenges in multi-deep storage systems, offering a flexible and efficient solution for heterogeneous item configurations.
...
Result: The dataset provides granular details like bolus insulin delivery, rarely found in existing datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for concept drift often forget past knowledge, leading to declining prediction accuracy. The paper aims to retain and reuse past concepts when they recur.

Method: CEP stores forecasters for different concepts, retrieves the nearest forecaster for test samples, and evolves new models for new concepts while eliminating outdated knowledge.

Result: Experiments show CEP effectively retains knowledge of different concepts and significantly improves prediction accuracy in recurring concept scenarios.

Conclusion: CEP successfully addresses recurring concept drift by retaining and utilizing past knowledge, enhancing online forecasting performance.
...
<details>
  <summary>Details</summary>
Motivation: To enhance zero-shot prediction of protein properties using PLMs without additional training.

Method: Inject a dropout layer (0.1 rate) between the embedding and transformer layers during inference, averaging outputs like Monte-Carlo dropout.

Result: Improved zero-shot performance on ProteinGym, even without prior dropout training.

Conclusion: Dropout at inference boosts PLM performance, offering a simple, training-free enhancement.
...
<details>
  <summary>Details</summary>
Motivation: Current pretraining of LLMs is computationally expensive and inefficient, prompting the need for a method to leverage existing pretrained models more effectively.

Method: AoE interpolates individual weight tensors from parent models to create child variants, enabling gradual or sharp transitions in model behavior.

Result: The method produces functional child models with predictable traits, demonstrated by the DeepSeek R1T 'Chimera' model, which combines traits of its parents efficiently.

Conclusion: AoE is a viable, efficient approach to creating capable child models from existing pretrained ones without additional fine-tuning or distillation.
...
<details>
  <summary>Details</summary>
Motivation: To understand how intelligent systems balance generalization and identification in their internal representations, and to quantify the fundamental limits of this tradeoff.

Method: The authors derive closed-form expressions for generalization and identification probabilities, extend the analysis to noisy and heterogeneous spaces, and validate the theory using minimal ReLU networks, CNNs, and vision-language models.

Result: The study reveals a universal Pareto front for the tradeoff, predicts a sharp collapse in multi-input capacity, and shows non-monotonic optima. Empirical results align with theoretical predictions.

Conclusion: Finite-resolution similarity is a fundamental constraint in intelligent systems, shaping the representational capacity of both artificial and biological networks.
...
<details>
  <summary>Details</summary>
Motivation: To improve time series forecasting by addressing computational inefficiencies of Transformer models and enhancing generalization through semantic priors and interpretable temporal encoding.

Method: Integrates semantic-aware embeddings (from pretrained language models) and spline-based Kolmogorov-Arnold Networks (KAN) within a selective state-space framework (Mamba model).
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between discrete optimization methods and their continuous-time approximations by focusing on discretization error, providing better theoretical tools for analyzing momentum-based optimization.
...
Result: A continuous-time model for the HB method with controlled discretization error, applied to study implicit regularization in directional smoothness and implicit bias in diagonal linear networks.

Conclusion: The work offers a refined continuous-time approximation for the HB method, with practical implications for understanding optimization in deep learning, supported by numerical experiments.
...
Result: VLMs show language-like prompt sensitivity in vision; InternVL2 models are notably robust. Sensitivity may relate to training data.
...
<details>
  <summary>Details</summary>
Motivation: Sparse and intermittent demand forecasting is challenging due to frequent zero-demand periods, impacting inventory management.

Method: Proposes a Model-Router framework with rule-based, LightGBM, and InceptionTime routers to assign suitable forecasting strategies based on demand patterns.

Result: Experiments show the InceptionTime router improves accuracy by 11.8% (NWRMSLE) and speeds up inference by 4.67x compared to single-model benchmarks.
...
Method: Uses modular neural networks where modules selectively combine previous policies with internal policies, enabling linear parameter growth with task count.
...
Method: TRAINCHECK infers invariants tailored for DL training and uses them to proactively detect errors, offering debugging assistance.

Result: The framework detected 18 out of 20 real-world silent errors in one iteration and found 6 unknown bugs in training libraries.
...
<details>
  <summary>Details</summary>
Motivation: Current methods like DXA scans are costly and time-consuming, and supervised models require large datasets, which are hard to obtain due to technical and legal challenges.

Method: The study uses a $p$-Laplacian regression model on 3D optical image biomarkers (e.g., height, volume) from 847 patients, comparing it with supervised models like SVR.
...
Conclusion: The $p$-Laplacian model is a promising alternative for healthcare applications, especially in data-constrained environments.
...
Method: Train smaller models using GRPO, a simple reward structure, simplified tool-calling, extra tokens for tool results, and a data mix favoring visually difficult examples.

Result: Improved performance on VQA tasks compared to similarly-sized baselines.

Conclusion: Combining GRPO, tool use, and strategic training data enhances VLM performance in detailed visual reasoning.
...
<details>
  <summary>Details</summary>
Motivation: The study addresses the critical need for accurate SSD health prediction to ensure data reliability, aiming to overcome the generalization limitations of traditional models.

Method: The model integrates BiGRU for bidirectional temporal feature extraction and multi-head attention for dynamic feature weighting, enhancing sensitivity to critical health indicators.

Result: The model achieves 92.70% and 92.44% accuracy on training and test sets, with a 0.94 AUC, demonstrating robust performance and generalization.

Conclusion: The BiGRU-MHA model offers a practical solution for SSD health prediction, supporting preventive maintenance and intelligent decision-making in storage systems.
...
<details>
  <summary>Details</summary>
Motivation: Healthcare data is often incomplete or limited, and ML models lack transparency, hindering their effectiveness in critical areas like AMD diagnosis.

Method: Developed CACTUS, a tool for AMD stage classification that incorporates genetic, dietary, clinical, and demographic factors, ensuring explainability and flexibility.

Result: CACTUS outperforms standard ML models, identifies key factors, and provides confidence in results, aiding decision-making and reducing bias.

Conclusion: CACTUS enhances AMD diagnosis by improving classification accuracy and transparency, offering a reliable tool for clinicians.
...
<details>
  <summary>Details</summary>
Motivation: To enhance one-class classification by utilizing available negative samples without requiring labels during optimization.

Method: Proposes GRKneg, a kernel improvement for OC-SVM using negative samples, and compares it with standard OC-SVM and binary SVM.

Result: GRKneg outperforms standard OC-SVM with RBF kernel and binary SVM when few negative samples are available.

Conclusion: GRKneg is effective for small-scale one-class classification with limited negative samples, offering better performance than traditional methods.
...
<details>
  <summary>Details</summary>
Motivation: Online learning in VFL faces challenges due to asynchronous data generation and non-intersecting feature sets, which prior research overlooked.

Method: An event-driven framework activates only relevant clients per event, incorporating dynamic local regret (DLR) for non-convex models in non-stationary environments.

Result: The framework outperforms existing online VFL methods in stability and efficiency, reducing communication and computation costs.

Conclusion: The proposed event-driven online VFL framework effectively addresses real-world challenges, offering a practical solution for asynchronous data scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Previous federated black-box prompt tuning methods ignored the high query costs of cloud-based LLMs, prompting a need for a more efficient approach.

Method: The study introduces FedOne, a framework that degrades FedAvg to activate one client per round, optimizing query efficiency in federated black-box prompt learning.

Result: Numerical experiments confirmed FedOne's superior query efficiency, aligning with theoretical predictions.

Conclusion: FedOne effectively addresses the query cost issue in federated black-box prompt tuning, offering a practical solution for cloud-based LLM services.
...
<details>
  <summary>Details</summary>
Motivation: Anomaly Detection Systems using Autoencoders face challenges due to nonstandardized separation thresholds, impacting detection performance.
...
Result: Not explicitly stated in the abstract, but the approach aims to improve detection performance.

Conclusion: Automating threshold definition with machine learning can enhance the effectiveness of anomaly detection in Autoencoders.
...
Method: Characterizes channels in the loss landscape using gradient dynamics, geometry, and functional interpretation, focusing on divergence of weights and formation of gated linear units.

Result: Gradient flow solvers often reach these channels, which appear as flat minima but lead to gated linear units, revealing unexpected computational capabilities of fully connected layers.

Conclusion: The study provides insights into quasi-flat regions in loss landscapes and highlights the emergence of gated linear units as a key feature of neural network optimization.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of reliable, scalable RL reward signals for general reasoning beyond math and code.

Method: Developed Guru, a corpus of 92K verifiable examples across six reasoning domains, with domain-specific reward design and filtering.

Result: RL's effectiveness varies by domain; Guru-7B and Guru-32B outperform baselines by 7.9% and 6.7% on a 17-task evaluation suite.

Conclusion: RL can facilitate genuine skill acquisition, especially for domains with limited pretraining exposure, and Guru models advance open-source reasoning capabilities.
...
Conclusion: ODD improves error estimation reliability and accuracy by addressing the limitations of DIS², with practical results available on GitHub.
...
<details>
  <summary>Details</summary>
Motivation: Early prediction of MS disability is difficult due to disease heterogeneity, prompting the use of advanced models to integrate diverse data.

Method: Employed tabular and time-series foundation models, a custom multimodal transformer, and machine learning on clinical and digital Floodlight data.

Result: Multimodal models outperformed unimodal ones, with the best performance from a transformer using Moment FM embeddings.

Conclusion: Multimodal approaches and foundation models enhance early MS disability prediction, with potential applications in other complex diseases.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of ensuring fair outcomes across agents while maximizing system performance in multi-agent settings with limited reward information.
...
Result: Outperforms baseline methods in fairness and efficiency, validated through synthetic and real-world experiments.

Conclusion: The proposed MA-MAB framework effectively balances fairness and performance, with strong empirical results.
...
<details>
  <summary>Details</summary>
Motivation: To address the overlooked challenge of training instability in sequential representations for sepsis treatment RL, which negatively impacts policy performance.

Method: Uses CDE state representation with early stopping or stabilization methods and enforces acuity-aware representations via correlation regularization with clinical scores (SOFA, SAPS-II, OASIS).

Result: Stable CDE autoencoder produces representations strongly correlated with acuity scores, enabling superior RL policies (WIS return > 0.9). Unstable training leads to degraded performance (WIS return ~ 0).

Conclusion: Training stability is critical for effective CDE-based representation learning in clinical RL, with practical guidelines provided for encoding irregular medical time series.
...
Result: SFT-GO outperforms baselines across benchmarks, datasets, and base models, showing robustness and effectiveness.

Conclusion: SFT-GO improves learning dynamics and performance by adaptively emphasizing challenging token groups.
...
<details>
  <summary>Details</summary>
Motivation: To address the unrealistic assumption in $μP$ theory that vocabulary size is fixed, and to understand its effect on training dynamics.

Method: Theoretical analysis of vocabulary size's effect, leading to identification of the LV regime, followed by experimental validation including pretraining a 1B model.

Result: Optimal embedding LR to hidden LR ratio scales as $Θ(\sqrt{width})$ in the LV regime, differing from $μP$'s $Θ(width)$.

Conclusion: The LV regime provides more accurate scaling rules for LLMs with large vocabularies, validated by experiments.
...
Conclusion: SPG offers a scalable, efficient alternative to traditional hyperparameter optimization, demonstrating consistent performance gains.
...
Method: Uses staged training to refine networks by learning residuals sequentially, optimizing loss functions, parameters, and adaptive data sampling.

Result: Validated effectiveness through benchmark problems, showing improved accuracy.

Conclusion: HiPreNets offers a structured approach to enhance neural network precision, addressing key training challenges.
...
<details>
  <summary>Details</summary>
Motivation: To understand the extent, triggers, and responses of hallucinations in LLMs when used in embodied agents for long-horizon tasks, especially under scene-task inconsistencies.

Method: Constructed a hallucination probing set from an existing benchmark, evaluated 12 models across two simulation environments, and analyzed their reasoning and failure modes.
...
Conclusion: The study highlights fundamental limitations in LLMs for infeasible tasks and provides actionable insights for developing more reliable planning strategies.
...
<details>
  <summary>Details</summary>
Motivation: Missing traffic data due to sensor failures and sensing gaps hinders reliable traffic modeling, necessitating a robust solution.

Method: NCPF integrates CP decomposition into neural architecture with learnable embeddings and hierarchical feature fusion, using Hadamard products and MLPs for nonlinear refinement.

Result: NCPF outperforms six baselines on six urban traffic datasets, demonstrating superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Address the lack of unified taxonomy and systematic framework for verifying machine unlearning, driven by privacy, security, and legal compliance needs.

Method: Proposes a taxonomy categorizing verification into behavioral and parametric methods, analyzes existing techniques, and identifies vulnerabilities.

Result: A structured survey of verification methods, revealing gaps and vulnerabilities, and laying groundwork for future research.

Conclusion: Highlights open problems and calls for robust, efficient, and theoretically grounded unlearning verification mechanisms.
...
Conclusion: SKIL provides a principled account of how complex behaviors emerge from dynamic combinations of fundamental motor primitives.
...
TL;DR: The paper evaluates ML techniques for solving the 1D Poisson equation, emphasizing quantitative accuracy and generalization, revealing limitations and proposing a new cross-validation method.


<details>
  <summary>Details</summary>
Motivation: To rigorously assess ML's accuracy in scientific applications, focusing on differential equations like the 1D Poisson equation, where generalization is critical.

Method: Analyzes training dynamics, derives generalization bounds, and tests models (white-box, black-box, deep linear, shallow neural networks, DeepONets, Neural Operators) empirically.

Result: Generalization to true physical equations isn't guaranteed; models show opposing behaviors. Green's function representations can be extracted from black-box models.
...
Result: SoKA achieves comparable or superior performance with only 0.99M trainable parameters, 25% fewer than LoRA/PiSSA, and shows faster convergence.

Conclusion: SoKA is a robust and efficient PEFT method for large-scale model adaptation, offering significant parameter savings and performance benefits.
...
TL;DR: The paper explores the robustness of federated learning against Byzantine behavior, revealing a tradeoff between averaging and validity conditions. It provides theoretical bounds on centroid approximation and introduces a new algorithm for convex validity.
...
Method: The study analyzes validity conditions and their impact on centroid approximation, derives lower and upper bounds, and introduces a new algorithm for convex validity. Empirical evaluations in federated settings are also conducted.

Result: Lower bound of min{(n-t)/t, √d} on centroid approximation under box validity, and an upper bound of 2min{n, √d}. A new algorithm achieves √(2d)-approximation under convex validity.

Conclusion: The paper establishes theoretical bounds and an algorithm for robust federated learning, demonstrating their applicability in peer-to-peer settings and empirical evaluations.
...
TL;DR: The paper proposes a method to generate synthetic held-out data for Dataset Inference (DI), enabling detection of unauthorized dataset usage in LLM training without requiring real in-distribution held-out data.
...
Method: The approach involves (1) training a data generator on a suffix-based completion task to create diverse synthetic data matching the original distribution, and (2) post-hoc calibration to align likelihoods of real and synthetic data.

Result: Experiments show the synthetic data enables high-confidence DI detection of original training sets with low false positives, supporting copyright claims.

Conclusion: The method reliably addresses the held-out data challenge for DI, making it practical for real-world applications like litigation.
...
<details>
  <summary>Details</summary>
Motivation: The need for scalable infrastructure planning tools to address the conflicting requirements of diverse regions (urban, suburban, rural) as EV adoption grows.
...
Result: In Georgia, USA, it improved coverage by 12%, halved travel distance for low-income residents, and ensured power capacity, remaining scalable for national use.
...
TL;DR: Proposes a credit risk management framework for 3PL-led SCF in CBEC, using generative AI to assess risk and determine loan sizes.


<details>
  <summary>Details</summary>
Motivation: Addresses SMEs' financing challenges in CBEC by leveraging 3PL-led SCF and in-transit inventory as collateral.
...
Result: Validated through experiments, the framework effectively assesses credit risk and determines loan sizes.

Conclusion: Pioneers generative AI in CBEC SCF risk management, improving SME access to capital.
...
TL;DR: SecFwT is an MPC-based framework for efficient, privacy-preserving fine-tuning of LLMs, addressing computational overhead and inefficiency in softmax attention.


<details>
  <summary>Details</summary>
Motivation: Adapting LLMs to privacy-sensitive domains like healthcare and finance is challenging due to data scarcity and privacy constraints.
...
Conclusion: The framework enables scalable and secure fine-tuning of LLMs for privacy-critical applications.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of optimizing molecules for multiple therapeutic targets due to sparse rewards and conflicting constraints in drug discovery.

Method: Integrates Seq2Seq VAE into iterative loops, alternating between expanding chemical diversity and constraining molecules based on multi-target docking thresholds.
...
TL;DR: The paper analyzes in-context learning (ICL) with missing or incorrect labels, showing that multilayer transformers outperform single-layer models by leveraging unlabeled data effectively.


<details>
  <summary>Details</summary>
Motivation: To understand how ICL works with imperfect demonstrations and improve semi-supervised learning in tabular data.

Method: Theoretical study using binary Gaussian mixture models and partially labeled data, comparing single-layer vs. multilayer transformers.

Result: Multilayer transformers implicitly construct estimators that exploit unlabeled data, with performance scaling exponentially with depth.
...
TL;DR: Predicting abnormal lab test values using performed tests, focusing on CBC, by treating tabular data as a set translation problem, improving AUC by up to 8%.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis via predicting abnormal test values from routine tests like CBC, addressing missing data in clinical tables.

Method: Formulate tabular data as a set translation problem using GPT-like embeddings for labels and values, handling missing data without estimation.

Result: Achieved up to 8% AUC improvement for predicting high uric acid, glucose, cholesterol, and low ferritin levels.

Conclusion: The method effectively bridges LLMs with tabular clinical data, enhancing prediction accuracy without imputing missing values.
...
<details>
  <summary>Details</summary>
Motivation: Traditional KD methods fine-tune NNPs, increasing energy barriers and limiting high-energy structure data. A gentler, off-the-shelf teacher model is needed for better exploration.

Method: Two-stage training: student NNP trained with off-the-shelf teacher-generated data, then fine-tuned with a smaller DFT dataset.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models in FWI struggle with iterative sampling and heuristic output control, and lack clarity on integrating approximate velocity models. The paper aims to resolve these issues.

Method: The authors propose cI²SB, an extension of Image-to-Image Schrödinger Bridge, for conditional sampling between ground truth and smoothed velocity models, leveraging observed seismic signals.

Result: Experiments show cI²SB outperforms prior conditional diffusion models, achieving high sample fidelity with fewer neural function evaluations (NFEs) than supervised learning.

Conclusion: The cI²SB framework effectively addresses limitations of diffusion models in FWI, offering a more efficient and accurate solution for velocity model reconstruction.
...
TL;DR: The paper proposes a method to improve one-run DP auditing in black-box settings using stronger membership inference attacks (MIA) with quantile regression, achieving tighter bounds efficiently.
...
Method: The study incorporates quantile regression for stronger MIA to enhance one-run auditing in black-box settings, evaluated on DP-SGD-trained CIFAR-10 models.

Result: The proposed approach achieves tighter empirical lower bounds while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Applying DiTs to molecules introduces unique challenges, such as combining discrete graph data with continuous geometry and managing Euclidean symmetries.
...
Conclusion: The framework highlights the impact of architectural choices and symmetry priors, offering promising directions for large-scale molecular generative modeling.
...
TL;DR: A novel method for learning ground metrics in optimal transport improves performance in embedding, clustering, and classification tasks by leveraging class labels at the distribution level.


<details>
  <summary>Details</summary>
Motivation: Traditional ground metrics in optimal transport are either predefined (e.g., Euclidean) or learned in a supervised way, which often lacks generalization or handles limited distribution types.
...
Result: The learned metric enhances optimal transport distances, improving performance in tasks like embedding, clustering, and classification. Demonstrated on scRNA-seq data across diseases.

Conclusion: The approach addresses limitations of existing methods, offering a more flexible and interpretable solution for comparing distributions.
...
Method: Proposes a framework for exploration as entropy maximization over a data manifold defined by a pre-trained diffusion model, using density estimation and a mirror descent algorithm for fine-tuning.

Result: The algorithm converges to the optimal exploratory diffusion model under realistic assumptions and shows promising results in synthetic and high-dimensional text-to-image tasks.

Conclusion: The work provides a scalable and theoretically grounded approach for exploration in generative models, validated empirically.
...
TL;DR: The paper addresses optimizing vaccination strategies for disease spread (SIS model) on unknown graphs, proposing a learning algorithm for graph recovery and efficient vaccination methods.


<details>
  <summary>Details</summary>
Motivation: Existing works assume known graphs for optimal vaccination, but real-world scenarios often lack this knowledge. The paper aims to minimize disease extinction time with unknown graphs.

Method: The approach splits the problem into learning the graph (using an inclusion-exclusion-based algorithm) and vaccination strategy (optimal and greedy algorithms for SRM).

Result: The learning algorithm's sample complexity is established, and the vaccination algorithm runs in polynomial time for bounded treewidth graphs. Experiments validate the methods.

Conclusion: The proposed framework effectively handles unknown graphs, with theoretical guarantees and practical validation for learning and vaccination strategies.
...
TL;DR: The paper addresses the lack of standardized evaluation for Explainable AI (XAI) methods by introducing a unified framework (VXAI) through a systematic literature review.


<details>
  <summary>Details</summary>
Motivation: The opacity of black-box AI models like Deep Neural Networks undermines trustworthiness, necessitating transparent explanations (XAI) that require rigorous evaluation.

Method: A systematic literature review following PRISMA guidelines, analyzing 362 publications to aggregate metrics and propose a categorization scheme.

Result: Identified 41 metric groups and introduced a three-dimensional framework for evaluating XAI methods.

Conclusion: The VXAI framework offers a structured, comprehensive approach to standardize XAI evaluation, enhancing comparability and future research.
...
Result: Provides a comprehensive overview of reward modeling, including established and emerging techniques.
...
TL;DR: PRISM introduces a framework for identifying and scoring polysemantic features in neural networks, improving feature description accuracy and capturing multiple concepts per neuron.


<details>
  <summary>Details</summary>
Motivation: Current methods assume neurons are monosemantic, limiting their ability to describe complex, polysemantic features. PRISM addresses this gap.
...
Result: PRISM outperforms existing methods in description quality and polysemanticity capture, as shown by improved scores.

Conclusion: PRISM enhances interpretability by better capturing the complexity of neural network features, especially polysemantic ones.
...
Method: Evaluate memory-based zero-shot RL in domains with partial observability of states, rewards, and dynamics.

Result: Memory-based methods outperform memory-free baselines.
...
TL;DR: The paper identifies a vulnerability in aligned LLMs where safety protections can be undermined by fine-tuning, even with benign data. It proposes LoX, a training-free method to enhance safety robustness by extrapolating low-rank subspaces, reducing attack success rates significantly.


<details>
  <summary>Details</summary>
Motivation: Addressing the vulnerability of aligned LLMs to safety breaches during fine-tuning, despite alignment efforts.
...
Result: LoX reduces attack success rates by 11% to 54% against benign and malicious fine-tuning attacks, while maintaining task adaptability.

Conclusion: LoX effectively enhances safety robustness by moving LLM parameters to a flatter, less perturbation-sensitive zone, validated by empirical results.
...
Result: Experiments on eight datasets show RHO outperforms state-of-the-art methods by effectively capturing diverse homophily.

Conclusion: RHO robustly learns varying homophily patterns in normal nodes, improving anomaly detection performance.
...
Result: AutoRule improves win rate by 28.6% on AlpacaEval2.0 and 6.1% on MT-Bench, with reduced reward hacking.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods lack qualitative comparison and fail to convey broader structural relationships, making it hard to understand shifts or speed changes.
...
Conclusion: The method improves qualitative understanding of time series comparisons by visualizing key transformations.
...
TL;DR: A user-guided projection framework for image and text data leverages MLLMs for customizable, interpretable visualizations via natural-language prompts, enhancing cluster separation and interactivity.
...
Method: Uses zero-shot classification with MLLMs and natural-language guiding prompts to dynamically steer projections.

Result: Improves cluster separation and transforms DR into an interactive, user-driven process.
...
TL;DR: The paper investigates dense latents in sparse autoencoders (SAEs), showing they are meaningful and persistent, not just training artifacts. It analyzes their geometry, function, and origin, revealing their functional roles in language models.


<details>
  <summary>Details</summary>
Motivation: To address concerns that dense latents in SAEs are undesirable artifacts and to understand their true nature and role in language models.

Method: Systematic investigation of dense latents, including their geometry, function, and origin, through ablation studies, taxonomy creation, and layer-wise analysis.

Result: Dense latents are persistent and meaningful, forming antipodal pairs and serving roles like position tracking, context binding, and output signals. They evolve from structural to semantic to output-oriented features across layers.

Conclusion: Dense latents are intrinsic to language model computation and should not be dismissed as noise, as they serve functional roles.
...
TL;DR: LIT-LVM improves linear predictor accuracy by modeling interaction terms with low-dimensional latent vectors, outperforming elastic net and factorization machines, especially in high-dimensional settings.


<details>
  <summary>Details</summary>
Motivation: Linear predictors with interaction terms can model non-linear relationships, but accurately estimating these terms is challenging, especially in high-dimensional scenarios.

Method: LIT-LVM represents features with low-dimensional latent vectors, adding structured regularization to mitigate overfitting.

Result: LIT-LVM achieves better prediction accuracy than elastic net and factorization machines, particularly with many interaction terms.

Conclusion: LIT-LVM provides accurate interaction term estimation and useful low-dimensional feature representations.
...
TL;DR: A framework certifies pixel-level robustness for black-box attribution methods using randomized smoothing, ensuring reliable explanations despite input perturbations.


<details>
  <summary>Details</summary>
Motivation: Post-hoc attribution methods are non-robust to small input perturbations, undermining trust. This work aims to provide guarantees for pixel-level robustness.

Method: Uses randomized smoothing to sparsify and smooth attribution maps, reformulating the task as a segmentation problem to certify pixel importance against perturbations.

Result: Certified attributions are robust, interpretable, and faithful across 12 methods and 5 ImageNet models.

Conclusion: The framework enables reliable use of attributions in downstream tasks, addressing robustness concerns.
...
<details>
  <summary>Details</summary>
Motivation: Research on adversarial attacks in tabular data is fragmented; this paper aims to consolidate and guide future work.

Method: Conducts a literature review to categorize attack strategies and analyze real-world applicability.

Result: Identifies key trends, attack categories, and practical challenges in adversarial attacks on tabular models.

Conclusion: Provides a structured overview to aid future research in mitigating adversarial vulnerabilities in tabular ML.
...
TL;DR: The paper formalizes spatiotemporal over-squashing in STGNNs, showing its unique challenges and revealing counterintuitive propagation biases, with implications for efficient design.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored issue of over-squashing in Spatiotemporal GNNs (STGNNs) and understand its distinct impact compared to static GNNs.

Method: Formalizes the problem, analyzes convolutional STGNNs, and compares time-and-space vs. time-then-space processing paradigms.
...
Conclusion: Provides insights into STGNN dynamics and guidance for more effective designs, validated on synthetic and real-world datasets.
...
Method: RePCS compares output distributions of parametric and retrieval-augmented paths using KL divergence, requiring no model access or retraining.
...
Method: Uses simple linear algebra tools to derive bias and variance bounds, avoiding complex PSD matrix manipulations.
...
Conclusion: The simplified analysis enhances accessibility and may aid in improving training methods for realistic models.
...
TL;DR: The paper extends Gold's inductive inference framework to include computational observations and restricted input sources, overcoming learning barriers for general recursive functions. It introduces new observation types and connects learning computable functions to finite-state transducer inference, but shows limitations for linear-time computable functions.


<details>
  <summary>Details</summary>
Motivation: To study the learnability of general recursive functions under realistic constraints by extending traditional frameworks with computational observations.
...
Result: Overcomes learning barriers for general recursive functions with new observation types, but shows no computable or polynomial-mass characteristic sets exist for linear-time computable functions.

Conclusion: The framework advances learning computable functions under constraints, revealing connections to finite-state transducer inference, but highlights inherent limitations for certain function classes.
...
Method: Empirical analyses and direct interventions to stabilize gradient flow, tested on various agents and environments.

Result: Simple, effective interventions enable strong performance even at large scales.

Conclusion: The proposed solutions are easy to implement and compatible with existing algorithms, offering a practical approach to scaling deep reinforcement learning.
...
TL;DR: LogNet, a logic gate-based framework, enhances interpretability and performance of DL-based indoor localization by identifying influential APs and diagnosing noise impact, achieving lower error, smaller model size, and reduced latency.
...
Method: Introduces LogNet, a logic gate-based framework, to interpret DL models by identifying key APs and analyzing noise impact.

Result: LogNet improves performance with lower localization error (1.1x-2.8x), smaller model size (3.4x-43.3x), and reduced latency (1.5x-3.6x).

Conclusion: LogNet provides transparent reasoning for DL-based localization, enhancing stability and adaptability for long-term deployments.
...
<details>
  <summary>Details</summary>
Motivation: To enhance AI efficiency and sustainability by mimicking human compositional reasoning.
...
Result: On 110 SJTU-TMQA meshes, MicroRicci reduces iterations (2.4x speedup), tightens curvature spread, and improves UV-distortion-to-MOS correlation (r = -0.93), with minimal runtime overhead (0.25 ms per iteration).

Conclusion: MicroRicci's efficiency, automatic adaptation, and high-quality results make it ideal for real-time applications in graphics and simulation.
...
Method: DP-GRAPE introduces three modifications: privatizing gradients after projection, using random Gaussian matrices instead of SVD-based subspaces, and applying projection during backpropagation.

Result: DP-GRAPE reduces memory usage by over 63% for Vision Transformers and 70% for RoBERTa-Large compared to DP-Adam, with similar performance. It also scales to large models like OPT (6.7B parameters).
...
<details>
  <summary>Details</summary>
Motivation: Fairness in ML is crucial as AI impacts society, but biased training data leads to unfair outcomes. Addressing label bias is essential for trustworthy systems.
...
Conclusion: GFLC effectively addresses label bias while maintaining fairness, offering a robust solution for fairness-aware ML.
...
TL;DR: Federated learning (FL) is effective for BrainAGE estimation in stroke patients, outperforming single-site models and closely matching centralized learning. BrainAGE correlates with vascular risks and functional outcomes.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns in training robust BrainAGE models by evaluating FL's performance and exploring BrainAGE's association with clinical outcomes in stroke patients.

Method: Used FLAIR images from 1674 patients across 16 centers, comparing centralized learning, FL, and single-site models. Analyzed BrainAGE's link to vascular risks and functional outcomes via logistic regression.

Result: FL outperformed single-site models and was close to centralized learning. BrainAGE was higher in diabetic patients and significantly associated with post-stroke recovery.

Conclusion: FL is viable for BrainAGE without data centralization. BrainAGE's link to vascular risks and recovery suggests its prognostic value in stroke care.
...
Method: Introduces CAWR, combining robust loss functions for policy optimization and advantage-based prioritized replay to filter poor explorations.
...
<details>
  <summary>Details</summary>
Motivation: The difficulty in accurately identifying graphical implicit correlations in multimodal irony detection tasks motivates the development of a more effective model.

Method: SemIRNet uses ConceptNet for common-sense reasoning, cross-modal semantic similarity modules at word and sample levels, and contrastive learning for feature optimization.

Result: The model achieves 88.87% accuracy and 86.33% F1, outperforming existing methods by 1.64% and 2.88%, respectively.

Conclusion: Knowledge fusion and semantic similarity detection are crucial for improving multimodal irony detection performance.
...
<details>
  <summary>Details</summary>
Motivation: To address persistent challenges in MLLMs' visual fine-grained perception and commonsense causal inference.

Method: Introduces Argus Inspection benchmark and the Eye of Panoptes framework with a binary parametric Sigmoid metric for holistic evaluation.

Result: Experiments on 26 MLLMs show the highest performance in visual fine-grained reasoning is only 0.46.

Conclusion: The research provides insights for further refinement of MLLMs.
...
TL;DR: A hybrid AI model combining ConvNeXt and EfficientNet outperforms traditional methods in classifying falcon diseases like Normal, Liver Disease, and Aspergillosis.


<details>
  <summary>Details</summary>
Motivation: To improve falcon health surveillance by accurately diagnosing diseases, ensuring safety during hunting.

Method: Uses a hybrid ConvNeXt and EfficientNet AI model trained on a large dataset, evaluated by accuracy, precision, recall, and F1-score.

Result: The hybrid model surpasses traditional diagnostic methods and standalone models in performance.

Conclusion: This AI model advances precise falcon disease detection and supports future AI-driven avian healthcare innovations.
...
Method: ViLLa integrates three components: visual detection for identifying animals, a language parser for understanding queries, and symbolic reasoning for logic-based inference.

Result: The system effectively answers queries about animal presence, count, and location, demonstrating interpretability and modularity.

Conclusion: ViLLa successfully bridges visual content with human-interpretable queries, offering a transparent alternative to black-box models for animal monitoring.
...
TL;DR: The paper introduces GraphGSOcc, a model combining semantic and geometric graph Transformers for 3D semantic occupancy prediction, addressing feature aggregation and boundary ambiguities in 3DGS methods.


<details>
  <summary>Details</summary>
Motivation: To resolve issues in existing 3DGS methods: lack of semantic correlation in feature aggregation and boundary ambiguities due to missing geometric constraints.

Method: Proposes Dual Gaussians Graph Attention for dynamic dual graph structures (geometric and semantic) and Multi-scale Graph Attention for fine-to-coarse feature optimization.

Result: Achieves 24.10% mIoU on SurroundOcc dataset with 1.97% improvement and 13.7% memory reduction (6.1 GB GPU memory).

Conclusion: GraphGSOcc effectively improves semantic occupancy prediction by integrating semantic and geometric constraints, outperforming prior methods.
...
<details>
  <summary>Details</summary>
Motivation: The need for reliable, fine-grained detection of AI-generated videos with persuasive evidence for auditors and users.

Method: Creation of the DAVID-X dataset with defect-level annotations and rationales, and development of the DAVID-XR1 model for interpretable reasoning.

Result: The model achieves strong generalization across various generators and modes, offering transparent detection.

Conclusion: Explainable detection methods like DAVID-XR1 enhance trustworthiness in identifying AI-generated videos.
...
TL;DR: A survey of recent deep learning-based multi-agent trajectory prediction methods (2020-2024), focusing on architectural designs, input representations, and prediction strategies, with insights into challenges and future directions.
...
Method: Review and categorization of recent deep learning-based methods, focusing on architectural design, input representations, and prediction strategies, with evaluation on ETH/UCY benchmark.

Result: Identifies key advancements and categorizes methods, highlighting their strengths and limitations.

Conclusion: The survey outlines challenges and future research directions for improving multi-agent human trajectory prediction.
...
TL;DR: The paper introduces ArchForms-4000, a dataset of human and machine-generated 3D forms, and ArchShapeNet, a CNN model for classifying them. The model outperforms humans in distinguishing form origins, revealing insights for improving generative design tools.


<details>
  <summary>Details</summary>
Motivation: The need to objectively analyze differences between human-designed and machine-generated 3D forms to advance generative design tools.

Method: Created ArchForms-4000 dataset (2,000 human and 2,000 machine-generated forms) and developed ArchShapeNet, a 3D CNN with a saliency module for classification.

Result: ArchShapeNet achieved 94.29% accuracy, 96.2% precision, and 98.51% recall in distinguishing form origins, outperforming human experts.

Conclusion: Human-designed forms excel in spatial organization and detail, while the study provides insights for enhancing generative tools.
...
TL;DR: MonoVQD improves monocular 3D detection by integrating denoising into DETR, addressing gradient vanishing, and using self-distillation for better query refinement.


<details>
  <summary>Details</summary>
Motivation: Overcoming limitations of DETR-like architectures in monocular 3D detection for optimal performance.
...
Conclusion: MonoVQD advances monocular 3D detection with scalable and effective components.
...
TL;DR: ChartIR improves chart-to-code generation in MLLMs by using structured instructions and iterative refinement, outperforming other methods.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with chart-to-code generation due to the complexity of translating visual elements into structured code.

Method: ChartIR uses structured instructions (description and difference) for visual understanding and decomposes the task into initial code generation and iterative refinement.

Result: ChartIR achieves superior performance on models like Qwen2-VL and GPT-4o compared to other methods.

Conclusion: The proposed iterative refinement method enhances MLLMs' chart-to-code generation capabilities effectively.
...
TL;DR: PictSure is an ICL framework for few-shot image classification that focuses on optimizing image embeddings, outperforming existing models on out-of-domain tasks.
...
<details>
  <summary>Details</summary>
Motivation: Kernel size in CNNs is often overlooked but impacts performance. The paper aims to optimize it systematically.
...
Conclusion: BKSEF redefines kernel size as an optimizable parameter, offering practical and theoretical benefits for CNN design.
...
Result: Achieves human-level accuracy, reduces annotation costs by 2x, and requires manual verification for less than 5% of frames.

Conclusion: The approach significantly enhances retail video annotation efficiency, benefiting applications like shopper analysis and security monitoring.
...
TL;DR: A novel active view selection (AVS) method using UPNet, a lightweight neural network, predicts uncertainty maps to select the most informative viewpoints for 3D reconstruction, achieving high accuracy with fewer views and reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying the minimal set of views for accurate 3D reconstruction in computer vision, avoiding the inefficiencies of traditional methods like NeRF or 3D Gaussian Splatting.

Method: Introduces UPNet, a feedforward deep neural network, to predict uncertainty maps from a single input image, guiding viewpoint selection. Aggregates uncertainty maps to suppress redundant views and select the most informative ones.

Result: Achieves comparable reconstruction accuracy with half the viewpoints, reduces computational overhead by 400x, and lowers CPU, RAM, and GPU usage by over 50%. Generalizes to novel object categories without additional training.

Conclusion: The proposed AVS method is efficient, scalable, and generalizable, offering significant improvements in speed and resource usage while maintaining high reconstruction quality.
...
<details>
  <summary>Details</summary>
Motivation: Alignment in T2I models is critical for capturing user intent while ensuring safety and fairness. Extending DPO from LLMs to T2I models addresses this need.

Method: DPO-Kernels integrate Hybrid Loss, Kernelized Representations (RBF, Polynomial, Wavelet), and Divergence Selection (Wasserstein, R'enyi). DETONATE benchmark evaluates alignment.

Result: DPO-Kernels improve alignment and robustness, demonstrated via DETONATE. AQI measures latent-space separability, revealing vulnerabilities.

Conclusion: DPO-Kernels enhance T2I alignment, with DETONATE and AQI providing tools for evaluating and improving model fairness and safety.
...
TL;DR: PeRL is a reinforcement learning approach for interleaved multimodal tasks, improving learning efficiency and performance by simulating varied positional relationships and filtering trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for vision-language models struggle with multi-image positional reasoning, limiting their applicability to complex real-world scenarios.

Method: Proposes PeRL with permutation of image sequences for diversity and a rollout filtering mechanism for effective policy exploitation.
...
Conclusion: PeRL addresses the limitations of current RL methods, achieving state-of-the-art results in multimodal reasoning tasks.
...
TL;DR: The paper proposes a Frequency-Calibrated Reconstruction Error (FCRE) method for Membership Inference Attacks (MIAs) on medical image diffusion models, addressing challenges in existing methods by focusing on mid-frequency reconstruction errors.


<details>
  <summary>Details</summary>
Motivation: Privacy concerns arise from diffusion models in sensitive areas like medical imaging, with MIAs being a tool to quantify risks. Existing methods struggle due to inherent image difficulty and high-frequency reconstruction issues.

Method: The FCRE method focuses on mid-frequency reconstruction errors, excluding high and low frequencies. It analyzes the reverse diffusion process, computes structural similarity scores, and compares them to a threshold for membership determination.

Result: Experiments show FCRE outperforms existing MIA methods on medical image datasets.

Conclusion: The FCRE method effectively mitigates challenges in MIAs for medical image diffusion models, offering improved performance.
...
Method: Uses simulated 2012 CMS Open Data to construct multi-channel jet-view images from energy deposits and tracks, testing ViTs and hybrids like ViT+MaxViT and ViT+ConvNeXt.

Result: ViT-based models outperform CNNs in F1-score, ROC-AUC, and accuracy, demonstrating the advantage of capturing long-range spatial correlations in jet substructure.

Conclusion: The study establishes a framework for ViT-based jet classification and provides a dataset for future deep learning research in this domain.
...
TL;DR: Proposes two models (LRCNs and Transformer) using RGB tactile images from GelSight for accurate compliance prediction, outperforming baselines and highlighting challenges with harder objects.


<details>
  <summary>Details</summary>
Motivation: Traditional compliance detection lacks portability and scalability, while existing neural methods suffer from low accuracy.

Method: Uses LRCNs and Transformer architectures with GelSight RGB tactile images to predict compliance metrics.

Result: Models show significant performance improvement over baselines; harder objects are more challenging to estimate.

Conclusion: Proposed models effectively estimate compliance, though harder objects pose difficulties.
...
<details>
  <summary>Details</summary>
Motivation: Historical maps contain valuable georeferenced text, but extraction is hindered by lack of effective methods and training data, especially for varied map styles and complex backgrounds.

Method: PALETTE introduces hyper-local sampling for precise feature learning and hyper-local positional embeddings for spatial interactions. SynthMap+ generates synthetic training data.

Result: PALETTE with SynthMap+ outperforms SOTA text spotters on benchmark datasets, especially for long and angled text, and has processed 60,000+ maps with 100M+ labels.

Conclusion: PALETTE and SynthMap+ provide a robust solution for text extraction from historical maps, enabling large-scale processing and improved accuracy.
...
TL;DR: StyleWallfacer is a unified framework for style transfer, enabling artist-level results and text-driven stylization through semantic-based style injection, data augmentation, and a training-free triple diffusion process.


<details>
  <summary>Details</summary>
Motivation: To address issues in traditional style transfer methods and unify frameworks for different tasks, enabling high-quality, drift-free style injection and text control.
...
Conclusion: StyleWallfacer revolutionizes style transfer with artist-level results and unified task handling.
...
Method: The authors employ the Wasserstein distance to align the distributions of features and code vectors, ensuring better representation and utilization of the codebook.
...
Conclusion: The proposed method effectively mitigates training instability and codebook collapse in vector quantization, improving performance and reducing information loss.
...
TL;DR: SynPo improves few-shot medical image segmentation by enhancing negative prompts in LVMs like SAM, using a novel Confidence Map Synergy Module and independent K-means clustering for point selection.


<details>
  <summary>Details</summary>
Motivation: Existing training-free methods using LVMs underutilize negative prompts, leading to poor performance on low-contrast medical images.

Method: SynPo combines DINOv2 and SAM for a reliable confidence map, selects top-k positive and negative points via Gaussian distribution and K-means clustering, and uses these as prompts for SAM.

Result: SynPo matches state-of-the-art training-based few-shot methods in performance.

Conclusion: SynPo effectively leverages negative prompts in LVMs for improved few-shot medical image segmentation without training.
...
TL;DR: The paper proposes the Point Distribution Set Abstraction (PDSA) module to improve point cloud analysis by correcting feature distribution in aggregation, enhancing efficiency and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing methods for aggregating local structures in point clouds face issues like irrelevant point interference, feature hierarchy gaps, high computational overhead, and noise sensitivity.

Method: PDSA uses high-dimensional space correlation to correct feature distribution, employs a lightweight cross-stage structural descriptor, and introduces a key point mechanism to optimize computation.
...
Conclusion: PDSA effectively addresses limitations of prior methods, offering better computational efficiency and robustness, validated by ablation and visualization results.
...
TL;DR: Echo-DND, a dual-noise diffusion model, improves LV segmentation in noisy echocardiograms using Gaussian and Bernoulli noises, multi-scale fusion, and spatial coherence calibration, outperforming SOTA models with Dice scores of 0.962 and 0.939.
...
Result: Achieves Dice scores of 0.962 (CAMUS) and 0.939 (EchoNet-Dynamic), surpassing existing models.
...
TL;DR: ReSeDis unifies corpus-level retrieval with pixel-level grounding, addressing both image relevance and object localization in visual search.


<details>
  <summary>Details</summary>
Motivation: Existing methods either focus on retrieval or grounding, but not both, limiting practical applications in large-scale visual search.

Method: Introduces ReSeDis, a task combining retrieval and grounding, with a curated benchmark and a joint metric for evaluation.

Result: A zero-shot baseline using a frozen vision-language model shows potential but leaves room for improvement.

Conclusion: ReSeDis provides a realistic testbed for advancing robust and scalable multimodal search systems.
...
TL;DR: The paper explores training generalist models for retinal OCT using visual in-context learning (VICL), enabling task flexibility without task-specific development. It proposes an evaluation protocol and benchmarks VICL performance.


<details>
  <summary>Details</summary>
Motivation: Specialized models for medical image analysis are limited to predefined tasks, requiring expertise and resources. Generalist models offer flexibility by allowing task definition on the fly.

Method: The study uses visual in-context learning (VICL) to train models for retinal OCT, generalizing across tasks with few examples at inference time.

Result: The work benchmarks VICL performance on multiple OCT datasets, establishing a baseline and highlighting its potential and limitations.

Conclusion: The paper advocates for generalist models in OCT, releases code for further research, and underscores the promise of VICL despite current challenges.
...
TL;DR: The paper proposes Privacy-Shielded Image Compression (PSIC), a method to protect images from exploitation by vision-language models while maintaining compression functionality.


<details>
  <summary>Details</summary>
Motivation: To safeguard user privacy against advanced vision-language models that can exploit publicly posted images.

Method: Introduces PSIC, which generates bitstreams with multiple decoding options, a CLTG module for bias generation, and UAEO optimization for leveraging VLP model uncertainty.

Result: Demonstrates effectiveness in protecting images while preserving perceptual quality and compression functionality.

Conclusion: PSIC is a plug-and-play solution that enhances privacy without compromising image utility.
...
TL;DR: The paper proposes a two-stage diffusion model (DM-FNet) for multimodal medical image fusion, improving feature capture and cross-modal interaction to enhance fused image quality.


<details>
  <summary>Details</summary>
Motivation: Existing MMIF methods struggle with detailed feature capture and cross-modal interaction, leading to suboptimal fusion results.

Method: A two-stage approach: Stage I uses a diffusion process with UNet for image reconstruction; Stage II inputs noisy images into a fusion network with adaptive modules. A hybrid loss function optimizes brightness, color, contrast, and detail.

Result: The method outperforms others in objective metrics, producing fused images with balanced brightness, rich textures, and clear edges.

Conclusion: DM-FNet effectively addresses limitations in MMIF, offering high-quality fused images for improved medical diagnosis.
...
TL;DR: Video-SALMONN 2 is an advanced audio-visual LLM with LoRA for video captioning, using DPO and a novel MrDPO approach to improve accuracy, outperforming GPT-4o and Gemini-1.5-Pro.


<details>
  <summary>Details</summary>
Motivation: Enhancing video understanding by generating detailed and accurate natural language descriptions.

Method: Uses LoRA and DPO with a novel multi-round DPO (MrDPO) approach, updating the reference model and LoRA module periodically, and incorporating ground-truth captions.

Result: Reduces captioning error rates by 28%, outperforms GPT-4o and Gemini-1.5-Pro, and maintains competitive performance on video QA benchmarks.

Conclusion: Video-SALMONN 2 is highly effective for video captioning, achieving state-of-the-art results with fewer parameters.
...
TL;DR: Proposes C-AFBiFPN, a feature enhancement and fusion framework for SAR ship detection, improving accuracy for small targets and multi-scale adaptability.


<details>
  <summary>Details</summary>
Motivation: Address challenges in SAR ship detection like scale variations, small vessel noise, and complex backgrounds.

Method: Introduces Convolutional Feature Enhancement (CFE) and integrates BiFormer attention into BiFPN for AFBiFPN, enhancing feature representation and fusion.

Result: Improves detection accuracy for small targets, robustness against occlusions, and multi-scale feature adaptability on SSDD.

Conclusion: C-AFBiFPN effectively addresses SAR ship detection challenges, offering superior performance in feature enhancement and fusion.
...
TL;DR: RA-NeRF improves camera pose estimation for 3D reconstruction, handling complex trajectories better than existing methods.
...
Method: RA-NeRF combines NeRF with photometric consistency, flow-driven pose regulation, and an implicit pose filter to enhance pose accuracy and robustness.
...
Conclusion: RA-NeRF is a robust solution for accurate camera pose estimation and scene reconstruction, even with complex trajectories.
...
TL;DR: RetroMem introduces a recall-augmented COD architecture with dynamic memory to improve camouflage object detection by leveraging historical context.
...
Conclusion: The dynamic memory approach in RetroMem enhances COD performance by effectively utilizing historical knowledge.
...
TL;DR: The paper explores Domain Adaptation (DA) techniques in the semiconductor sector, proposing the DBACS approach to improve defect classification in Industry 4.0/5.0 applications, validated on real-world Electron Microscope images.


<details>
  <summary>Details</summary>
Motivation: High demand and competition in semiconductors make time-to-market and quality critical. DA reduces manual re-labeling and computational costs while leveraging deep learning success in defect classification.
...
Result: The proposed DBACS approach proves effective in advancing DA techniques for semiconductor applications, validated on real-world data.

Conclusion: DA, especially with DBACS, offers a scalable and robust solution for defect classification in semiconductors, reducing resource costs and improving efficiency.
...
TL;DR: MSNeRV, a multi-scale feature fusion framework, improves neural video representation by enhancing temporal consistency and leveraging multi-scale features, outperforming VTM-23.7 in dynamic scenarios.
...
Method: Proposes MSNeRV with temporal windows for consistency, GoP-level grids for background, a multi-scale spatial decoder, and a multi-scale feature block for hidden features.
...
Conclusion: MSNeRV addresses INR limitations and advances video compression by integrating multi-scale and temporal features effectively.
...
Method: Combines a perception module (multi-view video analysis) with a reasoning module (action inference based on assembly state and task knowledge).

Result: Effective in challenging real-world scenarios with partial/uncertain observations, validated on LEGO and hydraulic press tasks.

Conclusion: ViMAT offers a marker-free, flexible solution for industrial assembly monitoring, proven in practical applications.
...
TL;DR: The paper discusses advancements in facial micro-expression (ME) analysis, highlighting the limitations of traditional separate spotting and recognition methods. It introduces two new tasks for MEGC 2025: ME-STR (unified spotting and recognition) and ME-VQA (visual question answering using MLLMs/LVLMs).


<details>
  <summary>Details</summary>
Motivation: Current ME analysis methods treat spotting and recognition separately, which is inefficient for real-world long-duration videos. The rise of MLLMs/LVLMs offers potential for improved ME understanding.

Method: Two tasks are proposed: ME-STR integrates spotting and recognition into a sequential pipeline, while ME-VQA uses MLLMs/LVLMs for ME-related visual question answering.

Result: The MEGC 2025 challenge will evaluate algorithms on these tasks, with results submitted to a leaderboard.

Conclusion: The paper aims to advance ME analysis by leveraging unified pipelines and multimodal models, addressing limitations of traditional approaches.
...
TL;DR: MapFM is an End-to-End model for online vectorized HD map generation, leveraging foundation models and multi-task learning for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: HD and BEV semantic maps are crucial for autonomous driving tasks like localization and planning, but current methods lack feature representation quality.

Method: MapFM integrates a foundation model for encoding camera images and adds auxiliary prediction heads for BEV semantic segmentation to enhance understanding.

Result: The model achieves higher accuracy and better quality in predicting vectorized HD maps due to richer contextual supervision.

Conclusion: MapFM demonstrates the effectiveness of combining foundation models and multi-task learning for superior HD map generation.
...
TL;DR: OpenPath is a novel open-set active learning method for pathology image classification, leveraging a vision-language model to efficiently select informative in-distribution samples while avoiding out-of-distribution data.
...
Result: Experiments on two public datasets show OpenPath outperforms state-of-the-art open-set AL methods, enhancing model performance through high-purity sample selection.

Conclusion: OpenPath effectively reduces labeling costs and improves efficiency in pathology image classification by addressing open-set challenges in active learning.
...
TL;DR: A new task of open-world object counting in videos is introduced, with CountVid model and VideoCount dataset for evaluation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of counting unique object instances in videos, especially in crowded scenes with occlusions and similar objects.
...
Conclusion: The task, model, and dataset are publicly available, showcasing effective open-world object counting in videos.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with geometric distortions in animal fur/skin patterns due to movement and posture changes.
...
Conclusion: The framework, requiring no ground truth UV annotations and trainable self-supervised, enhances re-identification accuracy across poses and angles.
...
TL;DR: DDIS introduces a Diffusion-assisted Data-free Image Synthesis method using a text-to-image diffusion model as an image prior to improve synthetic image quality, outperforming existing DFIS methods.
...
Result: DDIS outperforms prior DFIS methods, generating samples that better reflect the training data distribution, achieving state-of-the-art performance on PACS and ImageNet.

Conclusion: DDIS effectively addresses the limitations of DFIS by integrating diffusion models and alignment techniques, enhancing synthetic image quality for data-free applications.
...
Method: NERO uses neuron-level relevance clustering for ID classes, introduces a relevance distance metric, and incorporates scaled relevance and feature norms to enhance OOD separability.
...
Conclusion: NERO provides an effective and explainable solution for OOD detection in medical imaging, enhancing model reliability.
...
<details>
  <summary>Details</summary>
Motivation: The field of 3D AIGC is complex and largely inaccessible to non-experts, hindering broader adoption. This tutorial aims to bridge the gap by simplifying the process.
...
Result: Participants will gain the ability to finetune or develop 3D generative models for applications in gaming, VR, and industrial design.
...
TL;DR: MRG-LLM is a multimodal LLM combining a frozen LLM with a visual encoder and dynamic prompts for medical report generation, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Effective integration of LLMs with medical imaging data for accurate report generation remains underexplored.
...
Conclusion: MRG-LLM demonstrates superior performance in medical report generation, with code to be publicly released.
...
<details>
  <summary>Details</summary>
Motivation: The lack of large-scale 4D HOI datasets limits progress in text-driven human motion synthesis for interactions with objects.

Method: GenHOI uses Object-AnchorNet for sparse 3D HOI keyframe reconstruction and ContactDM with novel contact-aware components for dense 4D sequence interpolation.
...
Conclusion: GenHOI effectively overcomes dataset limitations and advances 4D HOI synthesis through its two-stage, contact-aware approach.
...
TL;DR: Summary of the NTIRE 2025 Shadow Removal Challenge with 306 participants and 17 final submissions, evaluated on fidelity and visual perception using the WSRD+ dataset.
...
Result: 306 participants registered, 17 teams submitted solutions, evaluated on diverse scenarios with self- and cast-shadows.

Conclusion: The challenge successfully assessed shadow removal methods, highlighting advancements in the field.
...
Method: CLAIM uses the SMILE module (clinically guided scar mask generation) and a diffusion-based generator, jointly training the segmentation network with the generator.
...
<details>
  <summary>Details</summary>
Motivation: Current dense metric depth estimation methods using radar require costly and data-intensive dense LiDAR supervision.
...
Result: RaCalNet reduces RMSE by 35.30% and 34.89% on datasets, producing detailed depth maps with clear contours.

Conclusion: RaCalNet offers a cost-effective, high-performance alternative to dense-supervised methods for radar-based depth estimation.
...
<details>
  <summary>Details</summary>
Motivation: To enhance multimodal understanding and generation by unifying visual representations and leveraging autoregressive and flow-based methods.

Method: Utilizes a 3D causal variational autoencoder with dual-path spatial-temporal fusion, autoregressive modeling for text, and flow matching for image/video generation. Includes a two-stage training approach.

Result: Show-o2 models effectively handle diverse multimodal tasks, demonstrating versatility in text, image, and video modalities.
...
TL;DR: A parameter-efficient semi-supervised framework for ultra-high-resolution (0.3m) land cover classification, leveraging SAM2 and a FreqWeaver Adapter, improves segmentation results with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in fine-grained land cover analysis due to high annotation costs, scale variation, and limited adaptability of large-scale vision models.

Method: Proposes a semi-supervised segmentation framework using SAM2 and a remote sensing-specific FreqWeaver Adapter for efficient fine-grained detail modeling.

Result: Achieves a 1.78% improvement over parameter-efficient tuning strategies and a 3.44% gain over state-of-the-art high-resolution remote sensing segmentation.

Conclusion: The framework effectively leverages unlabeled data and maintains minimal parameter overhead, delivering robust segmentation results with superior structural consistency.
...
TL;DR: A novel graph-based framework for end-to-end 3D tree reconstruction improves large-scale AGB estimation, reducing reliance on high-quality TLS data and pre-processing steps.
...
Method: Proposes a unified graph-based pipeline integrating tree segmentation, leaf-wood separation, and 3D skeletal reconstruction for end-to-end processing.

Result: Achieves strong performance in leaf-on (~20% error) and low-density ULS (~30% error) scenarios, validating scalability and robustness.

Conclusion: The framework enables scalable, non-destructive AGB estimation, reducing TLS dependency and promoting ULS use, advancing forest inventory and climate research.
...
TL;DR: The paper proposes DLoRAL, a Dual LoRA Learning paradigm for Real-VSR, combining Cross-Frame Retrieval and Consistency/Detail-LoRA modules to enhance spatial details while maintaining temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Existing SD-based Real-VSR methods often sacrifice spatial details for temporal consistency, leading to suboptimal visual quality. The goal is to extract degradation-robust temporal priors and enhance details without losing coherence.

Method: DLoRAL uses a Cross-Frame Retrieval (CFR) module and trains Consistency-LoRA (C-LoRA) for temporal representations, followed by Detail-LoRA (D-LoRA) for spatial enhancement. The two phases alternate iteratively.

Result: DLoRAL achieves strong performance in accuracy and speed, delivering consistent and detail-rich outputs in a single diffusion step.

Conclusion: The proposed method effectively balances spatial detail enhancement and temporal consistency, outperforming existing approaches.
...
Method: Proposes M2M-Reg, leveraging mono-modal similarity and GradCyCon regularizer for diffeomorphism, extending to semi-supervised settings.
...
<details>
  <summary>Details</summary>
Motivation: Address computational and memory constraints of dense point cloud reconstruction in existing 3D detection methods.

Method: Uses Cubify Anything for single-view detection, CLIP for semantics, and fusion modules (association and optimization) for multi-view consistency.
...
Conclusion: The reconstruction-free paradigm offers efficient, real-time 3D detection with strong generalization across diverse scenarios.
...
<details>
  <summary>Details</summary>
Motivation: HOI generation is challenging due to strict contact accuracy and diverse motion requirements, with current methods compromising between realism and physical correctness.

Method: HOIDiNi uses Diffusion Noise Optimization (DNO) in two phases: object-centric (hand-object contact) and human-centric (full-body motion refinement).

Result: HOIDiNi outperforms prior works in contact accuracy, physical validity, and quality on the GRAB dataset, enabling complex interactions like grasping and placing.

Conclusion: HOIDiNi successfully generates realistic and controllable HOIs from text prompts, balancing contact precision and motion naturalness.
...
TL;DR: A new benchmark for evaluating long-range embodied tasks in robotics is introduced, addressing the limitations of current VLMs in handling long-term memory and embodied challenges.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with long-term memory and embodied tasks like object manipulation and navigation, necessitating a dedicated benchmark for evaluation.

Method: A benchmark is developed in the Habitat simulator, featuring 60 tasks requiring memory and contextual awareness, with procedural extensions for scalability. Baselines integrate VLMs with navigation policies.

Result: The benchmark enables scalable evaluation of memory and reasoning in embodied settings, highlighting areas for improvement in VLM performance.

Conclusion: The proposed benchmark addresses gaps in evaluating memory-intensive embodied tasks, providing a foundation for advancing VLM capabilities in robotics.
...
TL;DR: Higher visual quality doesn't always improve MLLM performance; controlled degradations can boost accuracy. VQ-TTT, a lightweight adaptation module, dynamically adjusts images to align with model preferences, improving performance without extra data.


<details>
  <summary>Details</summary>
Motivation: To understand how visual quality affects MLLM responses and reconcile the paradox where degraded images sometimes outperform high-fidelity ones.

Method: Systematic study with controlled degradations and stylistic shifts on images. Introduces VQ-TTT, a module that modulates frequency content and fine-tunes shallow vision-encoder layers via LoRA.

Result: VQ-TTT improves accuracy across MLLMs and datasets without external models or extra training data.

Conclusion: Adaptive visual inputs, not universally "clean" images, are better for MLLMs, redefining visual quality standards for AI.
...
Method: ViMaR uses a temporal-difference value model and margin-aware reward adjustment in two stages: identifying high-value captions and selectively refining weak segments.

Result: ViMaR generates more reliable, accurate, and detailed captions with a 4× speedup, and generalizes well to unseen models, improving caption quality consistently.

Conclusion: ViMaR is a scalable, transferable, and self-improving inference-time decoding strategy for VLMs, enhancing both performance and efficiency.
...
TL;DR: A novel method for relighting images/videos by jointly estimating albedo and synthesizing relit outputs using video diffusion models, improving realism and generalization.


<details>
  <summary>Details</summary>
Motivation: Overcoming limitations of existing relighting models (data scarcity, error accumulation) to achieve better scene understanding and lighting effects.

Method: Joint albedo estimation and relit output synthesis via video diffusion models, trained on synthetic and real-world data.

Result: Strong generalization, superior visual fidelity, and temporal consistency compared to prior methods.

Conclusion: The approach effectively addresses relighting challenges, offering realistic results across diverse scenes and materials.
...
<details>
  <summary>Details</summary>
Motivation: Existing video datasets lack diversity, duration, and annotations needed for world exploration training.

Method: Collected 5,000+ hours of FPV and UAV videos from 100+ countries, annotated with location, scene, weather, etc., and developed a toolbox for processing.

Result: Demonstrated dataset quality and trained an interactive exploration model (YUME) using a subset.
...
TL;DR: ECAD uses a genetic algorithm to optimize caching schedules for diffusion models, improving speed and quality without modifying network parameters.
...
Method: Proposes Evolutionary Caching (ECAD), a genetic algorithm to learn efficient caching schedules using calibration prompts.

Result: ECAD achieves significant speedups (e.g., 2.58x on PixArt-alpha) and better quality (4.47 FID improvement) while generalizing to unseen resolutions/models.
...
Method: A manually annotated dataset of 14 clinically significant AEs with contextual attributes is created. Multiple models, including transformer-based ones, are evaluated across three annotation granularities.

Result: Transformer models excel in coarse-grained extraction (F1 = 0.943) but underperform in fine-grained tasks (F1 = 0.675), especially for rare events and complex attributes.

Conclusion: The dataset serves as a benchmark for AE extraction, highlighting challenges in detecting underrepresented AEs and nuanced clinical language, and is available via DataLoch.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use implicit constraints during training, leading to low-quality outputs during constrained decoding. BoostCD addresses this by explicitly combining constrained and unconstrained predictions.
...
Conclusion: BoostCD effectively improves structured NLP tasks by exploiting complementary errors in constrained and unconstrained decoding, demonstrating superior performance.
...
Result: CrEst outperforms baselines, improving accuracy by 26.86% and F1 score by 3.49%, even under high noise.

Conclusion: CrEst effectively enhances LLM performance by addressing context credibility, proving robust in noisy conditions.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for multi-document reasoning are scarce due to high annotation costs, despite its relevance to LLM capabilities.

Method: MDBench is created using a novel synthetic generation process involving LLM-assisted edits of structured seed knowledge, converted into natural text and QA examples.

Result: MDBench proves challenging for current LLMs and prompting techniques, even with short document sets.

Conclusion: The knowledge-guided generation technique enables targeted analysis of MD reasoning and can adapt to future modeling advancements.
...
Method: Empirical analysis using PIDD, evaluating six LLMs (four open-source, two proprietary) and three traditional ML models. Metrics: accuracy, precision, recall, F1-score. Prompting methods: zero-shot, one-shot, three-shot.

Result: Proprietary LLMs (GPT-4o, Gemini Flash 2.0) outperform open-source models. Gemma-2-27B achieves high accuracy and surpasses traditional ML models in F1-score. Performance varies with prompting strategies.

Conclusion: LLMs show promise for medical prediction tasks, but improvements in prompt engineering and hybrid approaches are needed for better healthcare predictions.
...
Method: Introduce a special memory token optimized through training on fixed sequences; evaluate across languages, token lengths, and model scales.

Result: Llama 3.1 8B successfully reconstructs sequences up to ~240 tokens in English and Spanish.
...
Method: Employed topic modeling for lexicon development and supervised learning classifiers to analyze 300,000 suicides (2002-2020).

Result: Identified 1,198 cases of chronic social isolation, with higher odds among men, gay individuals, and divorced persons. Found other predictors like divorce, child custody loss, and eviction.

Conclusion: The NLP-based method enhances surveillance and prevention of social isolation and loneliness in suicide cases.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for open-ended long-form generation often miss key aspects like coherence, style, or relevance, or are biased by pretraining data.

Method: PrefBERT is trained on diverse long-form response datasets with Likert-rated quality and provides semantic reward feedback for GRPO training.

Result: PrefBERT reliably evaluates varied long passages and aligns well with GRPO's verifiable rewards, yielding responses better aligned with human preferences.

Conclusion: PrefBERT effectively addresses the challenges of open-ended long-form evaluation and improves policy model training by providing superior reward signals.
...
TL;DR: The paper explores how learning-time choices in knowledge encoding affect the unlearning of factual knowledge in LLMs, finding paraphrased descriptions improve unlearning and highlighting challenges in unlearning specific knowledge from text chunks.


<details>
  <summary>Details</summary>
Motivation: The need for reliable post-hoc unlearning in LLMs due to privacy, outdated, or harmful content drives this study.

Method: Empirical investigation of learning-time knowledge encoding's impact on unlearning effectiveness.

Result: Paraphrased descriptions enhance unlearning, but unlearning individual knowledge from text chunks is difficult.
...
TL;DR: The paper introduces a Discourse-aware Clarification Module (DCM) and Contribution-aware Preference Optimization (CPO) to improve dialogue discourse parsing by resolving ambiguities and reducing errors, outperforming SOTA baselines.


<details>
  <summary>Details</summary>
Motivation: Ambiguities in dialogue discourse parsing due to linguistic features like omission and idioms challenge parsers, necessitating a solution to enhance performance.

Method: Proposes DCM with two reasoning processes (clarification type and discourse goal) and CPO to optimize clarifications and reduce errors.
...
Conclusion: The DCM and CPO framework significantly improves dialogue discourse parsing by addressing ambiguities and optimizing clarifications.
...
TL;DR: The CKD-EHR framework improves disease risk prediction by distilling knowledge from a fine-tuned large language model (Qwen2.5-7B) to a lightweight BERT model, achieving higher accuracy and efficiency.
...
Method: Knowledge distillation from a fine-tuned Qwen2.5-7B teacher model to a BERT student model using multi-granularity attention.

Result: 9% higher diagnostic accuracy, 27% improved F1-score, and 22.2x faster inference on MIMIC-III.

Conclusion: CKD-EHR enhances accuracy, speed, and resource efficiency, offering a practical solution for clinical settings.
...
Method: Decompose OD into MRG (generating diverse responses) and PS (selecting based on preference). Introduce o2mDial corpus, in-context learning, instruction-tuning, and new metrics.

Result: The framework improves response diversity and quality (up to 90%) in smaller LLMs, matching larger models' performance.

Conclusion: Explicitly modeling o2m via MRG and PS enhances OD generation, proving effective for smaller LLMs.
...
Result: Reduces fertility by ~10%, improving inference speed by 10% without performance loss in downstream tasks.
...
TL;DR: The paper investigates memory mechanisms in state-space language models, revealing primacy and recency effects through structured recall tasks. It identifies three key mechanisms: long-term memory via sparse channels, short-term memory via delta-modulated recurrence, and dynamic memory allocation based on semantic regularity.
...
Method: Structured recall tasks applied to the Mamba architecture, with analysis of long-term memory (sparse channels), short-term memory (delta-modulated recurrence), and dynamic memory allocation (semantic regularity).

Result: A U-shaped accuracy profile shows strong performance at sequence start and end. Targeted ablations and perturbations validate the findings on 1.4B and 7B parameter models.

Conclusion: The study uncovers three distinct memory mechanisms in state-space models, highlighting their interplay in shaping memory retention and forgetting patterns.
...
TL;DR: The study evaluates proprietary and open-source LLMs for text classification of SDGs, finding that smaller models with prompt engineering can match larger models like GPT.


<details>
  <summary>Details</summary>
Motivation: Tracking progress toward the UN's SDGs is challenging due to data complexity, necessitating efficient text classification tools.

Method: The study analyzes LLMs for SDG text classification and evaluates task adaptation techniques like Zero-Shot, Few-Shot Learning, and Fine-Tuning.

Result: Smaller models optimized via prompt engineering perform comparably to larger models like GPT.

Conclusion: Efficient prompt engineering can enable smaller LLMs to achieve results similar to larger models in SDG text classification.
...
TL;DR: ProtoReasoning improves reasoning in LLMs using abstract prototypes, showing cross-domain generalization and performance gains.


<details>
  <summary>Details</summary>
Motivation: To understand and enhance the cross-domain generalization capabilities of Large Reasoning Models by identifying shared abstract reasoning prototypes.

Method: Proposes ProtoReasoning, a framework with automated prototype construction, verification, and scalability, using Prolog and PDDL for logical and planning tasks.

Result: Achieves improvements: 4.7% on logical reasoning, 6.3% on planning, 4.0% on general reasoning, and 1.0% on mathematics.

Conclusion: Reasoning prototypes enhance generalization, validating their role as foundational for generalizable reasoning in LLMs.
...
TL;DR: MinosEval is a novel evaluation method for open-ended QA that distinguishes between factoid and non-factoid questions, using adaptive scoring and ranking strategies for better alignment with human judgments and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation metrics for open-ended QA (e.g., ROUGE, BERTScore) fail to capture semantic nuances, and current LLM-based methods lack interpretability and adaptability across question types.

Method: MinosEval classifies questions into factoid and non-factoid, applying adaptive key-point scoring for factoid questions and instance-aware listwise ranking for non-factoid ones.
...
Conclusion: MinosEval addresses limitations of current evaluation approaches by offering a tailored, interpretable method for open-ended QA.
...
Method: Introduces a parallel dataset for Basque and Spanish variants, conducts crosslingual and in-context learning experiments with encoder-only and decoder-based LLMs.

Result: Performance declines with linguistic variation, particularly in Basque; encoder-only models struggle with Western Basque, aligning with linguistic theory.

Conclusion: Linguistic variation, not lexical overlap, causes performance drops; encoder-only models face challenges with peripheral dialects like Western Basque.
...
<details>
  <summary>Details</summary>
Motivation: Addressing gaps in general large language models for historical text analysis in computational humanities and AIGC technology.
...
Result: Achieves F1=0.68 in relation extraction with Xunzi-Qwen1.5-14B and improves DeepSeek's F1 by 11% on C-CLUE, reducing hallucinations and enhancing interpretability.
...
TL;DR: TopClustRAG is a hybrid RAG system combining sparse/dense retrieval and K-Means clustering to improve answer quality in large-scale QA tasks, ranking 2nd in faithfulness and 7th in correctness.


<details>
  <summary>Details</summary>
Motivation: To enhance answer diversity, relevance, and faithfulness in large-scale question answering by leveraging clustering and multi-stage processing.

Method: Uses hybrid retrieval (sparse/dense indices), K-Means clustering, cluster-specific prompts for LLM, and intermediate answer filtering/reranking.

Result: Ranked 2nd in faithfulness and 7th in correctness on the FineWeb Sample-10BT dataset.

Conclusion: Clustering-based context filtering and prompt aggregation are effective for improving RAG systems.
...
TL;DR: Proposes Thunder-DeID, a framework for de-identifying court judgments in South Korea, addressing legal and technical challenges with a DNN-based pipeline.


<details>
  <summary>Details</summary>
Motivation: Current de-identification processes are inadequate for large-scale court judgments, and legal definitions of personal identifiers are vague, hindering technical solutions.

Method: Develops Thunder-DeID, including a Korean legal dataset, systematic PII categorization, and an end-to-end DNN-based de-identification pipeline.

Result: The model achieves state-of-the-art performance in de-identifying court judgments.

Conclusion: Thunder-DeID effectively addresses legal and technical challenges in de-identifying court judgments, offering a scalable solution.
...
TL;DR: The paper surveys LLM-based approaches for clinical trial-patient matching, highlighting their potential and current limitations in adoption, evaluation, and benchmarks.
...
Method: The survey analyzes existing LLM-based approaches, benchmarks, and evaluation frameworks for trial-patient matching, identifying gaps and challenges.

Result: LLMs show promise for consolidating knowledge in trial-patient matching, but current methods rely on proprietary models and weak benchmarks, limiting adoption.

Conclusion: The paper calls for improved benchmarks and evaluation frameworks to better leverage LLMs in clinical trial recruitment, outlining future research directions.
...
<details>
  <summary>Details</summary>
Motivation: Addressing poor LID performance for low-resource languages due to class imbalance and bias, often limited to single-domain data like the Bible.
...
TL;DR: DeVisE is a framework for evaluating clinical LLMs by testing their sensitivity to demographic and vital sign changes in ICU discharge notes. It reveals that zero-shot models reason more coherently, while fine-tuned models are stable but less responsive to clinical changes.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for LLMs in clinical settings often miss distinguishing genuine medical reasoning from superficial patterns, necessitating a more nuanced approach.

Method: DeVisE uses a dataset of ICU discharge notes (real-world and synthetic) with controlled counterfactuals for demographics and vital signs. It evaluates five LLMs under zero-shot and fine-tuned settings, assessing input-level sensitivity and downstream reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of global language models' comprehension of local socio-cultural contexts, particularly India's rich diversity.

Method: Created SANSKRITI, a dataset with 21,853 question-answer pairs covering 16 cultural attributes across India's 28 states and 8 union territories, and evaluated it on various language models.

Result: Significant disparities were found in models' ability to handle culturally nuanced queries, with many struggling in region-specific contexts.

Conclusion: SANSKRITI provides a comprehensive benchmark to assess and improve language models' cultural understanding, setting a new standard for evaluation.
...
TL;DR: COSMMIC is a novel comment-sensitive multimodal, multilingual dataset for nine Indian languages, integrating articles, images, and user comments to enhance summarization. It evaluates four configurations using advanced models like LLama3 and GPT-4.


<details>
  <summary>Details</summary>
Motivation: To address the lack of research in Indian languages for comment-aware multimodal and multilingual summarization, bridging gaps in resources and fostering inclusivity.

Method: Introduces COSMMIC dataset with 4,959 article-image pairs and 24,484 comments. Tests summarization and headline generation across four configurations (text, comments, images, combined). Uses LLama3, GPT-4, IndicBERT, and multilingual CLIP for evaluation.

Result: Identifies effective configurations for NLG tasks by integrating reader insights, filtering noise, and extracting image insights.

Conclusion: COSMMIC uniquely combines text, images, and user feedback, advancing NLP research for Indian languages and promoting inclusivity.
...
TL;DR: The paper introduces Targeted Lexical Injection (TLI), a fine-tuning method to improve lexical alignment in low-resource languages for LLMs, showing significant improvements in Swahili-English alignment.
...
Method: Uses Targeted Lexical Injection (TLI) with Low-Rank Adaptation (LoRA) and contrastive learning to fine-tune early-layer embeddings.

Result: TLI improves lexical alignment for Swahili-English word pairs by 28.08% (trained) and 28.32% (unseen), with high statistical significance.

Conclusion: TLI effectively enhances cross-lingual knowledge preservation in LLMs, offering a parameter-efficient solution for low-resource languages.
...
<details>
  <summary>Details</summary>
Motivation: MLLMs often exhibit systematic localization errors (hallucinations), reducing reliability in GUI interactions.

Method: Proposes a fine-grained evaluation framework, the Peak Sharpness Score (PSS), and Context-Aware Cropping to improve model performance.

Result: The framework and methods enhance interpretability and robustness of GUI agent behavior.

Conclusion: The proposed solutions provide actionable insights for improving MLLM reliability in GUI interactions.
...
TL;DR: AgentGroupChat-V2 is a novel multi-agent framework addressing challenges in architecture, generalizability, and performance for complex tasks, achieving superior results across domains.
...
Method: The framework introduces (1) a parallel hierarchical task decomposition, (2) an adaptive collaboration engine for dynamic LLM selection, and (3) optimization strategies for efficient problem-solving.

Result: Achieves 91.50% accuracy on GSM8K, 30.4% on AIME, and 79.20% pass@1 on HumanEval, with improvements scaling with task difficulty.

Conclusion: AgentGroupChat-V2 offers an efficient, general-purpose solution for LLM multi-agent systems, excelling in complex reasoning tasks.
...
<details>
  <summary>Details</summary>
Motivation: To clarify whether LLMs' high accuracy on reasoning benchmarks stems from true reasoning or statistical recall.

Method: Introduces RE-IMAGINE, a framework with a symbolic representation pipeline to generate problem variations across reasoning domains.

Result: Performance drops when LLMs face problem variations, indicating reliance on memorization.

Conclusion: LLMs depend partly on statistical recall, highlighting the need for further research on hierarchical reasoning skills.
...
TL;DR: CINGS is a post-training method that improves LLM grounding in external context by training with masked context prepended to responses, outperforming other methods in text and vision domains.


<details>
  <summary>Details</summary>
Motivation: LLMs often fail to ground responses in provided external context, leading to hallucinations or ungrounded outputs.

Method: CINGS trains models by prepending relevant context to responses but masking the context during loss computation, focusing learning on response tokens.

Result: CINGS-trained models show stronger grounding in text and vision domains, reducing hallucinations and maintaining factual consistency without degrading general performance.

Conclusion: CINGS effectively shifts model behavior to rely more on external context, improving grounding without sacrificing downstream task performance.
...
Result: Improves reasoning performance in offline RL fine-tuning and reward model training, with 2.6x efficiency gain.

Conclusion: SPARE offers a competitive and efficient solution for process supervision, with publicly released code and models.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods overlook emotion and associations in hyperbole and metaphor detection, limiting accuracy.

Method: EmoBi integrates emotion analysis, domain mapping, and bidirectional dynamic interaction for detection.

Result: Achieves 28.1% and 23.1% F1 score improvements on hyperbole and metaphor datasets, respectively.

Conclusion: EmoBi effectively advances hyperbole and metaphor detection by leveraging emotion and interaction.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of generating grounded and trustworthy responses in LLMs, particularly in scenarios where retrieval-augmented generation (RAG) fails.

Method: Uses GRPO (Group Relative Policy Optimization) with verifiable outcome-based rewards, focusing on answer correctness, citation sufficiency, and refusal quality. A two-stage training setup is employed.

Result: Reasoning-augmented models outperform instruction-only variants, especially in handling unanswerable queries and generating well-cited responses. Combining GRPO with GPT-4 distillation further enhances performance.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitation of local attention ignoring tokens outside the window, aiming for efficiency gains without performance loss.

Method: Introduces RATTENTION, combining local attention with a linear attention mechanism to handle out-of-window tokens.

Result: RATTENTION with a 512 window size matches full-attention performance and enhances long-context capabilities.

Conclusion: RATTENTION offers a superior efficiency-performance balance without compromising training speed.
...
<details>
  <summary>Details</summary>
Motivation: Modern language models often have open weights but closed training data, creating a need to approximate or recover useful training data from the model weights.

Method: A gradient-based approach is developed to select the highest-matching data from a large public text corpus, given only the weights of the original and finetuned models.

Result: The method improves performance from 65% to 80% on AG News classification and reduces perplexity from 3.3 to 2.3 on MSMARCO, nearing expert benchmarks.

Conclusion: The approach effectively recovers useful training data from model weights, demonstrating significant performance improvements in classification and supervised-finetuning tasks.
...
TL;DR: PredGen reduces latency in LLM-TTS systems by speculatively generating responses while the user is speaking, cutting delay by ~2x with minimal added cost.


<details>
  <summary>Details</summary>
Motivation: Address the noticeable latency in LLM-TTS systems, especially on consumer hardware, which degrades user experience.

Method: Proposes Predictive Generation (PredGen), a framework using speculative decoding to generate candidate responses during user speech.

Result: Simulated experiments show ~2x latency reduction with minimal additional computation.

Conclusion: PredGen effectively mitigates latency in LLM-TTS systems, enhancing real-time voice chat applications.
...
Result: Significant variations in gender inclusivity were found among the models, highlighting existing biases.

Conclusion: The study underscores the need for improving LLMs' gender inclusivity and sets a benchmark for future research.
...
Method: SciVer includes 3,000 expert-annotated examples from 1,113 papers, covering four reasoning types. It evaluates 21 state-of-the-art models using supporting evidence annotations.

Result: Models like o4-mini, Gemini-2.5-Flash, Llama-3.2-Vision, and Qwen2.5-VL show a significant performance gap compared to human experts.

Conclusion: The study highlights limitations in current models and provides insights for improving multimodal scientific comprehension and reasoning.
...
TL;DR: The paper introduces DiscoSG, a task for discourse-level text scene graph parsing, addressing limitations of single-sentence parsers. It presents DiscoSG-DS, a dataset, and DiscoSG-Refiner, a method improving performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current scene graph parsers fail to handle multi-sentence descriptions, missing cross-sentence phenomena like coreference, leading to fragmented graphs and poor VLM task performance.

Method: Proposes DiscoSG-Refiner: a two-step approach using small PLMs to draft and refine graphs iteratively, reducing overhead compared to large PLMs like GPT-4.

Result: DiscoSG-Refiner improves SPICE by ~30% over baselines, is 86x faster than GPT-4, and enhances downstream VLM tasks.

Conclusion: DiscoSG-Refiner offers a scalable, efficient solution for discourse-level scene graph parsing, outperforming existing methods while being resource-friendly.
...
TL;DR: WikiMixQA is a new benchmark with 1,000 MCQs for evaluating cross-modal reasoning over tables and charts from Wikipedia. It tests 12 VLLMs, showing proprietary models like GPT-4-o outperform open-source ones, but all struggle with long-context retrieval.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack focus on complex reasoning across multiple modalities in long documents, necessitating a new benchmark like WikiMixQA.

Method: Created WikiMixQA with 1,000 MCQs from 4,000 Wikipedia pages across seven topics, testing 12 VLLMs on cross-modal reasoning.

Result: Proprietary models (e.g., GPT-4-o) achieved ~70% accuracy with direct context but dropped below 50% with long-context retrieval. Open-source models maxed at 27%.

Conclusion: WikiMixQA highlights the challenges of long-context, multi-modal reasoning and serves as a key benchmark for advancing document understanding.
...
TL;DR: The paper explores using generative AI to automate MCQ creation in Portuguese, evaluating quality and challenges like semantic clarity and distractor design.


<details>
  <summary>Details</summary>
Motivation: Manual MCQ creation is time-consuming, and generative AI offers automation potential, but quality assessment, especially in non-English languages, is underexplored.

Method: The study generates Portuguese MCQs for reading comprehension, evaluates them via expert review and psychometric analysis of student responses.

Result: Current models produce MCQs comparable to human-authored ones but face issues in semantic clarity, answerability, and distractor quality.

Conclusion: Generative AI shows promise for MCQ automation in Portuguese, but improvements are needed in clarity and distractor design for real-world use.
...
TL;DR: The paper studies regret in LLMs, proposing methods to identify and analyze regret expressions, neurons, and representation layers, addressing key challenges with new metrics and workflows.


<details>
  <summary>Details</summary>
Motivation: Understanding regret in LLMs enhances model reliability and reveals neural cognition coding, but faces dataset, layer, and neuron analysis challenges.
...
Result: Identified optimal regret layer with S-CDI, discovered M-shaped decoupling pattern, and categorized neurons into regret, non-regret, and dual types.

Conclusion: The proposed metrics and workflow effectively address regret analysis challenges, improving understanding of LLM cognition and reliability.
...
Conclusion: Interpretation differences in cross-cultural communication involve a complex interplay between literal meanings and pragmatic factors.
...
<details>
  <summary>Details</summary>
Motivation: To assess LLMs' ability to follow instructions and generalize compositionally when generating sentences with ordered concepts, addressing biases and low diversity in outputs.

Method: Proposed Ordered CommonGen benchmark, measuring ordered coverage to evaluate LLMs' adherence to specified concept orders. Analyzed 36 LLMs for performance and biases.

Result: LLMs show understanding of instructions but exhibit biases, leading to low-diversity outputs. The top-performing LLM achieved only ~75% ordered coverage.

Conclusion: Ordered CommonGen highlights limitations in LLMs' instruction-following and compositional generalization, indicating areas for improvement.
...
TL;DR: The study evaluates six ML techniques for Romanian authorship attribution using the ROST corpus, with ANN achieving the highest performance using character n-grams.


<details>
  <summary>Details</summary>
Motivation: To address authorship attribution for Romanian texts, leveraging lightweight stylometric features for accuracy.

Method: Systematic evaluation of SVM, LR, k-NN, DT, RF, and ANN with character n-gram features.

Result: ANN performed best, achieving perfect classification in some cases with 5-gram features.

Conclusion: Simple character n-gram approaches can rival complex methods, especially in resource-constrained or under-studied languages.
...
<details>
  <summary>Details</summary>
Motivation: Large language models struggle with consistent and robust reasoning despite excelling in many tasks.
...
<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that reasoning traces are safe and highlight privacy risks in internal model thinking.

Method: Probing and agentic evaluations to analyze leakage in reasoning traces, focusing on test-time compute approaches.

Result: Increased reasoning steps amplify privacy leakage, revealing a tension between utility and privacy.
...
TL;DR: The paper evaluates 21 MT systems for their ability to preserve gender ambiguity in translations, finding most fail but a few use specific strategies.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of maintaining gender neutrality in grammatical gender languages during translation, avoiding misgendering and representational harms.
...
Result: Most MT systems lacked gender-neutral translations, though a few employed specific strategies depending on the target language.
...
<details>
  <summary>Details</summary>
Motivation: Deploying large VLMs on resource-constrained devices is challenging due to high computational demands, necessitating efficient distillation methods.
...
TL;DR: PhantomHunter is a detector for text from privately-tuned LLMs, addressing performance drops in existing methods by capturing family-level traits.


<details>
  <summary>Details</summary>
Motivation: The rise of privately-tuned LLMs causes performance drops in existing detectors, necessitating specialized solutions.

Method: PhantomHunter uses a family-aware learning framework to detect text from unseen, privately-tuned LLMs by focusing on shared traits.

Result: Outperforms 7 baselines and 3 industrial services with F1 scores over 96% on LLaMA, Gemma, and Mistral families.

Conclusion: PhantomHunter effectively detects text from privately-tuned LLMs, offering a robust solution to a growing challenge.
